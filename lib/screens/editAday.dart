...
class EditAday extends StatefulWidget {
  final String email;
  final String password;
  final String adsoyad;
  final int id;
  final Aday aday;
  const EditAday({Key? key, required this.email, required this.password, required this.adsoyad, required this.id, required this.aday,}) : super(key: key);

  @override
  State<EditAday> createState() => _EditAdayState();
}


class _EditAdayState extends State<EditAday> {
  ...
  int cocuksayisi=0;
  String? ad, soyad, email, yasadigi_sehir ,dogdugu_sehir,seciliMezuniyet, sonIsyeri,son_IsyeriSuresi, cv_url, /*yetkili*/cinsiyet ,askerlik,ehliyet,sigara,alkol,evlilik;
  ...
  void initState() {
    ...
    adController.text = widget.aday.ad.toString();
    soyadController.text = widget.aday.soyad.toString();
    telNoController.text = widget.aday.gsm.toString();
    mailController.text = widget.aday.email.toString();
    seciliMezuniyet = widget.aday.mezuniyet.toString();
    mezuniyetController.text = seciliMezuniyet.toString();
    yasController.text = widget.aday.yas.toString();
    dogumTarihiController.text = DateFormat('dd.MM.yyyy').format(DateTime.parse(widget.aday.dogumTarihi.toString()));
    yasadigiSehirController.text = widget.aday.yasadigiSehir.toString();
    dogduguSehirController.text = widget.aday.dogduguSehir.toString();
    sonIsyeriController.text = widget.aday.sonIsyeri.toString();
    toplamDeneyimController.text = widget.aday.toplamDeneyim.toString();
    cvController.text = widget.aday.cvUrl.toString();
    sonIsyeriCalismaController.text = widget.aday.sonIsyeriSuresi.toString();
    cinsiyetController.text=widget.aday.cinsiyet.toString();

    if (widget.aday.cinsiyet == 'Kadın') {
      kadinSecili = true;
      erkekSecili = false;
    } else if (widget.aday.cinsiyet == 'Erkek') {
      kadinSecili = false;
      erkekSecili = true;
    }
    print(cinsiyetController);
    print(widget.aday.cinsiyet);



    if (widget.aday.sigara != null) {
      sigaraVar = widget.aday.sigara !;
      sigaraYok = !widget.aday.sigara !;
      sigaraController.text = widget.aday.sigara ! ? 'True' : 'False';
      print(sigaraController);
    }

    if (widget.aday.askerlik != null) {
      yapildiSecili = widget.aday.askerlik !;
      yapilmadiSecili = !widget.aday.askerlik !;
      askerlikController.text = widget.aday.askerlik ! ? 'True' : 'False';
      print(askerlikController);
    }

    if (widget.aday.alkol != null) {
      alkolVar = widget.aday.alkol !;
      alkolYok = !widget.aday.alkol !;
      alkolController.text = widget.aday.alkol ! ? 'True' : 'False';
      print(alkolController);
    }

    if (widget.aday.ehliyet != null) {
      ehliyetVar = widget.aday.ehliyet !;
      ehliyetYok = !widget.aday.ehliyet !;
      ehliyetController.text = widget.aday.ehliyet ! ? 'True' : 'False';
      print(ehliyetController);
    }

    if (widget.aday.evlilik != null) {
      evlilikVar = widget.aday.evlilik !;
      evlilikYok = !widget.aday.evlilik !;
      evlilikController.text = widget.aday.evlilik ! ? 'Evli' : 'Bekar';
      print(evlilikController);
    }

    if (widget.aday.cocukSayisi != null) {
      cocukSayisiController.text = widget.aday.cocukSayisi.toString();
      cocuksayisi = int.tryParse(cocukSayisiController.text)!;
      print(cocukSayisiController);
    }
  ...
  void editAday() async{
    ...
    int? id=widget.aday.id;
    final response=await http.post(Uri.parse("$url/adayedit/$id"),
        body:{
          "ad": ad,
          "soyad": soyad,
          "gsm": telNo.toString(),
    ...
    print("Güncellenen aday :${response.body}");
    if(response.statusCode==201) {
      showDialog(context: context, builder: (BuildContext context){
        return AlertDialog(
          title: Text("Başarılı",),
          content: Text("Aday güncellenmiştir."),
          actions: [
      ...
      showDialog(context: context, builder: (BuildContext context){
        return AlertDialog(
          ...
          content: Text("Aday Güncellenirken hata oluştu. Lütfen tekrar deneyiniz."),
          actions: [
            ElevatedButton(onPressed: (){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>AdaylarPage(email: widget.email, password: widget.password, adsoyad: widget.adsoyad, id: widget.id,)));
            }, child: Text("Tamam"))
                  ...
                  Container(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Padding(
                        ...
                        child: Text("Aday Düzenle",style: TextStyle(fontSize: 18),),
                      ),
